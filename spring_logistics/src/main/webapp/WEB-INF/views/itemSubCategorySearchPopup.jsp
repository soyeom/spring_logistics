...
<c:choose>
    <c:when test="${not empty keyword}">
        <sql:query var="items" dataSource="${dataSource}">
            SELECT item_id, item_name, small_category, mid_category, big_category
            FROM item_master
            WHERE item_name LIKE '%' || ? || '%'
                OR small_category LIKE '%' || ? || '%'
                OR mid_category LIKE '%' || ? || '%'
        ...
        <c:choose>
            <c:when test="${not empty items.rows}">
                <c:forEach var="item" items="${items.rows}" varStatus="status">
                    <tr onclick="selectItem('${item.small_category}', '${item.item_name}')">
                        <td>${status.count}</td>
...
<script>
// JavaScript 함수 수정
function selectItem(smallCategoryCode, smallCategoryName) {
