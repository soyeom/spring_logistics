			    		...
			    		<c:forEach items = "${list}" var = "board">
					    	<tr>
							    <td class="text-center"
							        data-column1="${board.column1}"
							        data-column2="${board.column2}"
							        data-column3="${board.column3}"
							        data-column4="${board.column4}"
							        data-column5="${board.column5}"
							        data-column6="${board.column6}"
							        data-column7="${board.column7}"
							        data-column8="${board.column8}"
							        data-column9="${board.column9}"
							        data-column10="${board.column10}"
							        data-column11="${board.column11}">
							        <c:out value="${board.column1}" />
							    </td>
							    <td class="text-center">
...
<script>
	...
	function button_Click() {
	    ...
	    // td 안의 텍스트를 배열로 수집
	    var dataAttrs = Array.from(selectedRow.querySelectorAll("td")).map(td => td.dataset);
    	
	 	// 부모창 함수 호출 + 데이터 전달
		window.opener.out_Id_RowData(dataAttrs[0]);
