...
function filterTable() {
    ...
    rows.forEach(row => {
        const scName = row.cells[0].innerText.toLowerCase();
        let match = false;
        
        if(option === "all") {
            match = scName.includes(keyword)
        } else if(option === "name") {
            match = scName.includes(keyword);
        }
...
function selectSmallCategory(name) {

    window.opener.document.getElementById("smallCategory").value = name;
    window.close();
}
